...
export default { loginController, registerController, authController ,applyTeacherController};
*/
const userModel = require("../models/userModels");
const bcrypt = require("bcryptjs");
const jwt = require("jsonwebtoken");
...
const applyDoctorController = async (req, res) => {
  try {
    ...
    notifcation.push({
      type: "apply-Teacher-request",
      message: `${newDoctor.firstName} ${newDoctor.lastName} Has Applied For A Teacher Account`,
      data: {
...
try {
 ...
 const user = await userModel.findOne({_id: req.body.doctorInfo.userId});
  ...
  // Check if user has notifications property
  if (!user.notifcation) {
    user.notifcation = []; // Initialize if not exists
  }
  ...
  console.error('Error while booking appointment:', error.message);
  res.status(500).send({
    success: false,
